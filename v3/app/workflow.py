...
def _scan(node, state, logger, label, frame=None):
    ...
    # Diagnostics: always persist raw/mask/overlay (+ crops) and log the geometry
    # of every accepted card and the reason for every rejected contour. Without
    # this a failed scan leaves no artifact, so a "saw 4/5 cards" failure is blind.
    logger.save_debug_set(frame, result.mask, result.overlay, result.crops)
    for c in result.cards:
        logger.log(
            f"[{label}] accept bbox={c.bbox} aspect={c.aspect:.2f} solidity={c.solidity:.2f} score={c.score:.2f}"
        )
    for bbox, reason in result.rejected:
        logger.log(f"[{label}] reject bbox={bbox} reason={reason}")
...
def _cross_match(references, check_prepared):
    # Index-based positional assignment: reference slot s <-> check crop s-1.
    # detect_cards sorts BOTH scans' cards left-to-right by bbox center X, so the
    # i-th detection in either scan is the same physical card position on the floor.
    # We therefore map purely by POSITION. The previous greedy visual matching
    # (best compare_orientation_pair_multiscale score) swapped slots whenever two
    # cards looked near-identical (e.g. 8 of spades vs 8 of clubs), assigning
    # check #2 to slot 1 and check #1 to slot 2. Positional mapping cannot swap.
    # Returns {ref_slot: (check_index, score_normal, score_rot180)} for each pair.
    assign = {}
    for s in range(1, NUM_SLOTS + 1):
        ci = s - 1
        if ci >= len(check_prepared):
            continue
        rp = references.get(s)
        cp = check_prepared[ci]
