...
# ============================================================
# approach 1 — center-fine orientation (floating central pip match)
# ============================================================
...
def compare_center_fine_pair(ref_fine, check_fine):
    ...
    # DECISIVE signal (max |r - n|) in either direction. Isolating the pip is what
    # recovers the orientation of a near-symmetric card the global match cannot
    # resolve.
    #
    # Earlier this picked the margin maximizing (r - n), i.e. the strongest
    # *rotation* signal only. That estimator is one-sided: for a low-detail card
    # that is actually NORMAL, it cherry-picks the noisiest margin that happens to
    # look most rotated, producing a small spurious positive gap that can flip the
    # verdict to ROTATED_180. Selecting by max |gap| is symmetric — a normal card's
    # strongly-negative-gap margin can win and keep it NORMAL, while a genuinely
    # rotated card still surfaces its strongly-positive-gap margin.
    if ref_fine is None or check_fine is None:
        return (-1.0, -1.0)

    ref_rot = cv2.rotate(ref_fine, cv2.ROTATE_180)
    best_n, best_r, best_absgap = -1.0, -1.0, -1.0
...
def orientation_verdict(score_normal, score_rot180, min_good, margin,
    ...
    global_best = max(score_normal, score_rot180)
    if global_best < min_good:
        base = "LOW_CONFIDENCE"
    ...
    # Safety guard: the center-fine signal is only a TIE-BREAKER for a borderline
    # global verdict, never a substitute for a trustworthy one. When the global
    # match has itself collapsed below the floor (best structural score < min_good,
    # e.g. a strongly negative normal score against a barely-positive rot180), the
    # two crops are too unlike for the tiny central pip to carry any real meaning —
    # a noisy positive fine_gap there was blindly flipping genuinely unrotated cards
    # to a false ROTATED_180. Only let the fine result move the verdict once the
    # global base is at least minimally reliable; below the floor the honest answer
    # is LOW_CONFIDENCE.
    if global_best < min_good:
        return "LOW_CONFIDENCE"

    # approach 1 override: when the global verdict is not a confident ROTATED_180
